...
{block name="field"}
	...
	{else if $input.type == 'imagecate'}
		<div class="row"><div class="col-lg-6">
		<select>
			{foreach from=$selected_categories item=selected_category}
		...
		{foreach from=$selected_categories item=selected_category}
			{foreach from=$languages item=language}
				...
				<div class="form-group">
					<div class="col-lg-6">
						<input id="{$input.name}_{$language.id_lang}_{$selected_category}" type="file" name="{$input.name}_{$language.id_lang}_{$selected_category}" class="hide" />
						<div class="dummyfile input-group">
							<span class="input-group-addon"><i class="icon-file"></i></span>
							<input id="{$input.name}_{$language.id_lang}_{$selected_category}-name" type="text" class="disabled" name="filename" readonly />
							<span class="input-group-btn">
								<button id="{$input.name}_{$language.id_lang}_{$selected_category}-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
									<i class="icon-folder-open"></i> {l s='Choose a file' mod='blockbanner'}
								</button>
							</span>
						</div>
				...
				<div class="form-group">
					{foreach from=$test item=test1}
						{if $test1[0]['id_category'] == $selected_category}
							<img src="{$uri}img/{$test1[0]['image']}" class="img-thumbnail"/>
						{/if}
					{/foreach}
				</div>
				{if $languages|count > 1}
				...
				<script>
					$(document).ready(function(){
						$('#{$input.name}_{$language.id_lang}_{$selected_category}-selectbutton').click(function(e){
							$('#{$input.name}_{$language.id_lang}_{$selected_category}').trigger('click');
						});
						$('#{$input.name}_{$language.id_lang}_{$selected_category}').change(function(e){
							var val = $(this).val();
							var file = val.split(/[\\/]/);
							$('#{$input.name}_{$language.id_lang}_{$selected_category}-name').val(file[file.length-1]);
						});
					});
				</script>
			{/foreach}
