...
class Slideshow extends ObjectModel
{
	...
	public static $definition = array(
		...
		'primary' => 'id_posslideshows_slides',
		'multilang' => true,
		'fields' => array(
			'title' =>			array('type' => self::TYPE_STRING, 'validate' => 'isCleanHtml', 'size' => 255),
			'active' =>			array('type' => self::TYPE_BOOL, 'validate' => 'isBool', 'required' => true),
			'position' =>		array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt', 'required' => true),
			'cap_position' =>		array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'),

			// Lang fields
			'description' =>	array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isJson'),
	...
	public function add($autodate = true, $null_values = false)
	{
        ...
        //echo '<pre>'; print_r($this->description);die;
		$res = parent::add($autodate, $null_values);
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'posslideshows` (`id_shop`, `id_posslideshows_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			...
			WHERE `id_posslideshows_slides` = '.(int)$this->id
		);
	...
	public function getDescription(Slideshow $block, $id_lang = null)
    {
        if (!$id_lang) {
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_posslideshows_slides` = hs.`id_posslideshows_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_posslideshows_slides` as id_slide
			FROM `'._DB_PREFIX_.'posslideshows_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'posslideshows` hs ON (hss.`id_posslideshows_slides` = hs.`id_posslideshows_slides`)
